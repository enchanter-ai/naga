...
stdlib `ast`, runs **N1 Zhang-Shasha tree edit distance** over the parsed
AST against an empty tree to derive the node-count shape
...
| ID | Name                                  | Reference                                                                              |
|----|---------------------------------------|----------------------------------------------------------------------------------------|
| N1 | Zhang-Shasha Tree Edit Distance (structural AST) | Zhang K. and Shasha D. (1989), SIAM J. Comput. 18(6):1245-1262  |
...
**VF-08 (resolved) — N1 is now structural:** N1 previously flattened the AST to
its postorder node-type sequence and ran a Wagner-Fischer *string* edit
distance, which discarded structure — differently nested programs with the same
node-type multiset (e.g. `f(a, g(b))` vs. `f(g(a, b))`) collapsed to distance 0.
N1 now implements the Zhang-Shasha ordered-tree edit distance directly over the
parsed AST (LR-keyroots + forest-distance DP), so those cases score a non-zero
distance (2 for that example). The node label is the AST node *type*, so N1
measures structural shape, not identifier-level differences (that axis is N2/N3).
The persisted per-source `n1_signature` remains the tree's node count (distance
from the empty tree); the structural comparison runs on the pairwise call at
`/naga:match` and `/naga:validate`.
...
[shared/conduct/](shared/vis/conduct/) — loaded once into [CLAUDE.md](CLAUDE.md),
