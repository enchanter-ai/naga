...
$XDG_STATE_HOME/naga/<repo_id>/events.jsonl (fallback: naga/state/events.jsonl).

Usage (from Python):
    from publish import publish
    publish("naga.pattern.fingerprinted", {"source_path": ..., ...})

Usage (from shell — pipe JSON via stdin):
    echo '{"topic":"naga.pattern.fingerprinted","payload":{...}}' | python publish.py
...
def _events_path() -> Path:
    """Resolve the events.jsonl path: XDG_STATE_HOME/naga/<repo_id>/events.jsonl."""
    xdg = os.environ.get("XDG_STATE_HOME", "")
    if xdg:
        base = Path(xdg) / "naga" / _repo_id()
    else:
        # Fallback: naga/state/events.jsonl relative to this file's plugin root
        naga_root = Path(os.environ.get("CLAUDE_PLUGIN_ROOT",
                                        Path(__file__).resolve().parent.parent.parent))
        base = naga_root / "state"
...
def publish(topic: str, payload: dict) -> None:
    ...
    try:
        ...
        print(f"[naga:publish] failed to emit {topic!r}: {exc}", file=sys.stderr)
...
def _main() -> int:
    ...
    try:
        ...
        print(f"[naga:publish] bad stdin JSON: {exc}", file=sys.stderr)
        return 1
